...
from app.utils.common_objects import DeckBuilderActions
...
def can_user_add_card_to_deck(db_getter_connection, user_id, deck_id, card_id):
    ...
    # print(deck_stats)
    # print(card_count)
    if card_count and card_count >= 4 and card_info.get("card_class") != "energy":
        return False
...
def get_deck_data_html():
    ...
    print("User requested deck action, username:", session.get("username"))
    if json_request := request.get_json():
        ...
        if action_id == DeckBuilderActions.UPDATE_DECK.value:
            if deck_id:
                print("Updating Deck")
            ...
            action_id == DeckBuilderActions.ADD_CARD.value
        ...
        elif action_id == DeckBuilderActions.REMOVE_CARD.value:
            print("Removing Card")
            db_getter_connection.remove_card_from_deck(card_id, deck_id)
        elif action_id == DeckBuilderActions.LOAD_DECK.value:
            pass
        else:
        ...
        if not selected_deck_id and len(meta_data["deck_name_list"]) > 0:
            selected_deck_id = meta_data["deck_name_list"][0].get("id")
